...
construction, however, requires available nodes and edges that may not exist
for a newly uploaded garment.
...
observed item. The Polyvore Outfits disjoint split prevents item overlap across
its principal divisions and gives a more demanding test on unseen garments
\parencite{vasileva2018typeaware}. It does not remove shared photography,
category distributions, platform culture or labelling assumptions.
...
but cannot represent all clothing practices. Filtering official questions to
...
\parencite{efron1993bootstrap}. Its interval is conditional on those runs and
...
is evaluated under one deployed objective. It does not mean that the model has
identified an objectively best human outfit. A safety limit is required so an
...
of an isolated component \parencite{selbst2019fairness}. The proportionate
response is not to claim that bias has been solved, but to document limitations,
avoid identity inference, retain the selected garment, expose alternatives and
permit rejection or replacement.
...
\parencite{fawcett2006roc,guo2017calibration,bouthillier2021variance}. None
establishes human satisfaction.
...
Compatibility classification, completion ranking, calibration, search
completeness and user benefit are distinct claims. The first four can be
examined with secondary data and system experiments; user benefit cannot be
claimed without participant evidence. Chapter 3 specifies the methodology
